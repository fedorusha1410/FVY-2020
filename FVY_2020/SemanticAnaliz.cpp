	...
	bool Semantic::semanticsCheck(Lex::LEX& tables, Log::LOG& log)
	{
		...
		//Деление на ноль
			...
			case LEX_PROCENT:
			{
				...
				if (tables.lextable.table[i + 1].lexema == LEX_ID)
				{
					for (k; k > 0; k--)
					{
						if (tables.lextable.table[k].lexema == LEX_ID)
						{
							...
							{
								if (tables.lextable.table[k + 2].lexema == LEX_LITERAL && tables.idtable.table[tables.lextable.table[k + 2].idxTI].value.vbyte == 0)
								{
									sem_ok = false;
				...
				{
					if (tables.idtable.table[tables.lextable.table[i + 1].idxTI].value.vbyte == 0)
					{
						sem_ok = false;
			...
			case LEX_MORE:	case LEX_LESS:
			{
				...
				{
					if (tables.idtable.table[tables.lextable.table[i - 1].idxTI].iddatatype != IT::IDDATATYPE::UB)
						flag = false;
				}
			...
			case LEX_EQUALS:   case LEX_NOTEQUALS:
			{
				...
				{
					if (tables.idtable.table[tables.lextable.table[i - 1].idxTI].iddatatype == IT::IDDATATYPE::UB
						&& tables.idtable.table[tables.lextable.table[i + 1].idxTI].iddatatype == IT::IDDATATYPE::UB)
						flag = true;
