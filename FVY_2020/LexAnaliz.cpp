...
namespace Lex
{
	Graph graphs[N_GRAPHS] =
	{
		...
		{ LEX_OUTLINE, FST::FST(GRAPH_OUTLINE) },
		...
		for (int i = 0; i < ittable.size; i++)
		{
			if (ittable.table[i].idtype == IT::IDTYPE::L && ittable.table[i].iddatatype == type)
			{
				...
				case IT::IDDATATYPE::UB:
					if (ittable.table[i].value.vbyte == atoi(value))
						return i;
					break;
	...
	IT::IDDATATYPE getType(char* curword, char* idtype)
	{
		...
		if (!strcmp(TYPE_UBYTE, idtype))
			return IT::IDDATATYPE::UB;	 // числовой  ид
		if (isdigit(*curword) || *curword == LEX_MINUS)
			return IT::IDDATATYPE::UB;				// числовой литерал
		...
		bool& lex_ok)							// флаг ошибки(по ссылке)
	{
		...
		if (lex == LEX_LITERAL) // литерал
		{
			...
			if (int_ok && itentry->iddatatype == IT::IDDATATYPE::UB)
			{
				char p[11];
				_itoa(itentry->value.vbyte, p, 10);
		...
		else // идентификатор
		{
			...
			case IT::IDDATATYPE::STR:
				...
				break;
			case IT::IDDATATYPE::UB:
				itentry->value.vbyte = TI_UB_DEFAULT;
				break;
			case IT::IDDATATYPE::SYM:
	...
	bool analyze(LEX& tables, In::IN& in, Log::LOG& log, Parm::PARM& parm) {
					...
					case LEX_LITERAL:
					{
						...
						else // повторный идентификатор (уже есть)
						{
							...
							if (lexema == LEX_LITERAL)
							{
								...
								{
									int temp;
									if ((getType(id, in.words[i - 1].word)) == IT::IDDATATYPE::UB)
									{
		...
		if (flags % 2 != 0 && flag_main) {
			Log::WriteError(log.stream, Error::GetError(204));
			lex_ok = false;
		}
		return lex_ok;
	}
